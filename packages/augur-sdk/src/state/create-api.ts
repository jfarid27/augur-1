...
export async function create(ethNodeUrl: string, account?: string, enableFlexSearch = false): Promise<{ api: API, controller: Controller, bulkSyncStrategy: BulkSyncStrategy, logFilterAggregator: LogFilterAggregator }> {
  ...
  const controller = new Controller(augur, db, logFilterAggregator, bulkSyncStrategy);
  const api = new API(augur, db);

  return { api, controller, bulkSyncStrategy, logFilterAggregator};
}
